...
public class SchoolService {
	...
	/**
	 * '@Transactional'어노테이션을 사용하여 두 개 이상의 Entity들을 제어할 때 메서드가 하나의 트랜잭션 안에서 실행되도록 한다.
	 * 지연 로딩(LASY) 시 다른 Entity 정보를 함께 조회할 경우 proxy를 이용한 지연 로딩 처리 시 에러가 발생 하기 때문에 하나의 트랜잭션에서 실행한다.  
	 */
	@Transactional
