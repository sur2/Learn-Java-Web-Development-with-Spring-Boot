...
어노테이션(annotation)은 Java 1.5부터 지원되는 기능의 일종으로 메타데이터(Metadata)이다.

- 사전적 의미는 주석으로 컴파일 또는 런타임 시에 해석된다.
...
- ImportSelector : 어노테이션의 값에 따라서 로딩 여부가 결정되는 설정 클래스를 사용하기 위한 인터페이스
  어노테이션 값을 따르는 같은 빈을 두 개 등록했기 때문에 빈이 중복되었다. 빈이 오버라이드 되도록 아래의 설정을 application.properties에 추가한다.

  ```properties
  spring.main.allow-bean-definition-overriding=true
  ```

- @Conditional : 자바 설정 클래스를  선택할 수 있게 해주는 어노테이션